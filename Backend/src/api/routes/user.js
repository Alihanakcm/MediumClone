...
app.get("/user/:username", authToken.authenticateToken, (request, response) => {
    services.userService.getUser(request.params.username).then(data => response.send(data)).catch(err => console.log(err));
});
